...
// 前置声明 MonteCarloSimulator 的实现函数
SimulationResults runGBMSimulation() {
    ...
    int numSimulations = 1000;  // 减少模拟次数以加快速度
...
void runVasicekSimulation() {
    ...
    int numSimulations = 1000;  // 减少模拟次数
...
int main() {
    ...
    try {
        auto gbmResults = runGBMSimulation();
        ...
        // 打印几条路径的最终值
        std::cout << "\n前10条路径的最终值:" << std::endl;
        for (int i = 0; i < 10 && i < gbmResults.finalValues.size(); i++) {
            std::cout << "路径 " << i << ": " << gbmResults.finalValues[i] << std::endl;
        }
